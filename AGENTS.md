...
- 배포 운영 버전: **v3.13.4** (GitHub Pages, 2026-09-02). 검증된 릴리스 코드 커밋은 `2d8aff18fa7e4b88f636eb89776a5730c8001b12`이다.
  - index.html Git blob: `1016dc8f7c84018f94b614508e9a9ecc973e7df5`
  - sw.js Git blob: `6d1980fe5e39e0b82609bcd83bb630a1fc1f914f`
  - SW: `timegrid-v3.13.4-20260902`
  ...
  - 모바일 Todo는 이전의 compact two-tier 항목(최소 66px)과 v3.13.0의 한 줄 `할일 입력 | 영역 | 추가` composer를 사용한다. Routine은 72px two-tier와 `영역·활동 | 경과 | 요일` 분산 정렬을 사용한다. 카드의 추가 세로 패딩을 제거해 첫·마지막 행도 내부 분리선과 같은 행 패딩 리듬을 사용한다.
  ...
  - Tracker 활동 입력 폭은 확정 활동의 최종 굵기(600)를 적용한 뒤 단일 JS 경로에서 측정하고, 글꼴 준비 완료 후 다시 측정한다. 숨겨진 Tracker의 `clientWidth=0`은 폭 상한 계산에서 제외하고 `ResizeObserver`로 최초 표시·회전·split-view 때 재측정하며, 빈 입력은 CSS placeholder 안전폭을 가져 iOS 한글 잘림을 방지한다.
  - `[hidden]` 상태는 컴포넌트 display 규칙보다 우선한다. 계정 위젯은 인증 상태에 맞는 액션만 노출한다.
- v3.12.67의 `todoMutations` move/delete/restore 계약은 v3.13.4에도 유지된다.
