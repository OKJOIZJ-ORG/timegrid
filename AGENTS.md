...
Verified production baseline (2026-09-06): v3.14.11 (runtime commit `0f0393f4d384dd45f8fec0e1fb7d27bd477ccde5`, published on `main` at `5041b5e675895a89934a8a99602043a204c42742`).
- TimeGrid v3.14.11 deployed through Pages build 1197979342; all six public runtime assets exactly match Git bytes. The shared Area / Activity picker exposes empty, Area-only and Area-plus-Activity value kinds. Area-only uses compact optical padding and caret spacing on desktop and mobile; mobile has no state-independent width floor. Preserve v3.14.10 intrinsic grid ownership, the one-line composer and Area-plus-Activity full-label behavior. Both 390px and 1280px rendered Area-only width 58.97px with a 9px border-inclusive trailing inset and no clipping; all 28 Node programs and four real-bootstrap contexts passed. Native iPhone/WebKit and authenticated multi-device E2E remain unverified. Evidence: `RELEASE-2026-09-06-V3.14.11.md`.
- v3.14.10 established intrinsic mobile composer width: the title input owns the remainder, the classification picker follows content up to the genuine viewport bound, and Add retains its fixed minimum. Preserve `공부 · 정치와 법` full display and zero horizontal overflow.
- v3.14.9 modal entry belongs to `showDialog` and its one-shot class; do not restore persistent dialog[open] entrance animation. Run `tests/memo-popover.browser.mjs` for shared modal/popup changes. Earlier release reports remain historical.
- PWA workers own scoped navigation and declared static shell assets only; never intercept cross-origin live-data/auth streams or unknown endpoints. No update banner/button. Prepared updates activate at safe untouched entry/hidden transitions; only untouched, unblocked, positively identified different-build entry may reload once per target. Preserve editing, running/pending Stop guards, activated-shell HTML and mismatched-install rejection. Run `tests/pwa-lifecycle.test.mjs`, `tests/pwa-update.browser.mjs`, `tests/pwa-upgrade.browser.mjs`, `tests/pwa-long-fetch.browser.mjs` and real-bootstrap for this surface. Never restore a blind reload timer or reuse cache identities.
