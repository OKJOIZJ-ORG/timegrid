...
assert.match(html, /\.planner-card-todo \.add-row\{display:grid;grid-template-columns:minmax\(0,1fr\) max-content auto;[^}]*padding:10px 0/, "mobile Todo keeps one row while the semantic picker owns intrinsic width")
assert.match(html, /\.planner-card-todo \.add-row input\[type=text\]\{[^}]*grid-column:1[^}]*width:100%[^}]*min-width:0[^}]*min-height:40px/, "mobile Todo input yields the remaining width without forcing picker clipping")
assert.match(html, /\.area-picker\.cat-act\[data-value-kind="area"\]\{gap:4px;padding-inline:10px 8px;\}/, "area-only classification uses optical rather than symmetric trailing space")
assert.match(html, /btn\.dataset\.valueKind=act\?'activity':ar\?'area':'empty'/, "the shared picker exposes its semantic content state")
assert.match(html, /\.planner-card-todo \.add-row \.area-picker\{[^}]*grid-column:2[^}]*width:max-content[^}]*min-width:0[^}]*max-width:min\(48vw,180px\)[^}]*min-height:40px/, "mobile Todo picker follows its label without a state-independent minimum")
assert.match(html, /\.planner-card-todo \.add-row \.btn\{[^}]*min-width:56px[^}]*min-height:40px/, "mobile Todo add action uses the prior compact control height")
