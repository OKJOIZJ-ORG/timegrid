...
assert.match(html, /\.planner-card-todo \.add-row\{display:flex;align-items:center;[^}]*padding:10px 0/, "mobile Todo restores the compact one-row composer")
assert.match(html, /\.planner-card-todo \.add-row input\[type=text\]\{[^}]*flex:1 1 auto[^}]*min-height:40px/, "mobile Todo input stays compact and flexible")
